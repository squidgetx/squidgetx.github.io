...
let json = '/webmedia/muse/wordvecs10000.json'
// Create a new word2vec method

const wordVectors = ml5.word2vec(json, modelLoaded);

let input, button, greeting, pitchClass;
...
async function calculate(sentence) {
  ...
  let tokens = sentencefeatures.tokens.split(" ");
...
//  l//et similarityValues = similarityData.map(f => constrain(f[0], 0, 1) );
  ...
  */
  ...
  // Let's use stresses
  let tokenPitches = assignPitches(allPitches, pitches, tokens, featuremap);
  let tokenRhythms = assignRhythms(sentence, tokens, tokenPitches);
  let tokenHarmonies = assignHarmonies(pitches, featuremap, tokenPitches);
...
function assignPitches(allPitches, pitches, tokens, featuremap) {
    ...
    */
    ...
    let nextSkeletonTargetIndex = 0;
      ...
      console.log("pitch skeleton", pitchSkeleton);
    for(let i = 0; i < tokens.length; i++) {
      // Rule 0: every token gets the same pitch
      let t = tokens[i];
    ...
    console.log("token pitches", tokenPitches)
...
function assignRhythms(sentence, tokens) {
  ...
  console.log("---", tokenRhythms);
  return tokenRhythms;
}

let synth = new Tone.Synth({envelope : {
attack : 0.25 ,
decay : 0.1 ,
sustain : 0.3 ,
release : 1
...
function setup() {
  ...
  input.size(500, 300);
    input.position(displayWidth/2 - 250, 85);
    input.center('horizontal');

    button = createButton('submit');
    button.position(input.x + input.width/2, input.y + input.height + 12);
...
let textStr = "write a poem, hear a melody";

function draw() {
  background(255,255,255);
  textAlign(CENTER, TOP);
  textSize(32);
  text(textStr, width/2 - 10, 20)
}
