window.onload = () => {
  ...
  let pendulum_template = function(freq, multArr, s, freqMax=8) {
    const mult = multArr[0]/multArr[1]
    const Y_OFF = 16
    const LINE_OFF = 32 * 3
    const FPS = 30
    const RED = '#d43245'
    ...
    return {
        ...
        ui[1] = createSlider(p5, {
          ...
          onChange: (val) => {
            ...
            if (val != pendulum1.freq) {
              ...
              if (!useRhythm) {
                SYNTH.frequency.rampTo(val, 0.1)
                SYNTH2.frequency.rampTo(val * mult, 0.1)
              ...
              if (val < 15) {
                ...
                TONE_START = now + playNote2.interval/2

                playNote1.start(TONE_START)
                playNote2.start(TONE_START)
              } else {
                playNote1.interval = 1 / val
                playNote2.interval = 1 / (val * mult)
              }
      ...
      let enableAudio = function() {
        ...
        if (useRhythm) {
          ...
          SYNTH.triggerAttack(pendulum1.freq, Tone.now())
          SYNTH2.triggerAttack(pendulum2.freq, Tone.now())
        }
      }
      ...
      p5.draw = function() {
        ...
        // use the p5 clock 
        if (!TONE_ON) {
          updatePendulums(p5.frameCount/FPS)
        }
        p5.fill('grey')
        ...
        p5.stroke('grey')
        p5.strokeWeight(2)
        p5.line(p5.width*0.6, LINE_OFF, p5.width*0.6, p5.height - LINE_OFF)
        p5.pop()
        ...
        p5.text(pendulum1.freq.toFixed(2) + 'Hz', 10, 50)
        p5.fill(BLUE)
        p5.stroke(BLUE)
        p5.text(pendulum2.freq.toFixed(2) + 'Hz', 10, 80)
        p5.textSize(32)
        p5.fill('black')
        p5.stroke('black')
        p5.text(multArr.join(":"), 10, 240)
        p5.pop()
        ...
        for(let i = 0; i < 230; i++) {
          // Draw trails
          let t = (p5.frameCount - i)/FPS 
          if (TONE_ON) {
            t = Tone.now() - TONE_START + toneOffset - i / FPS;
          }

          p5.fill(RED)
          let th = Math.sin(t* Math.PI * pendulum1.freq + Math.PI) * Math.PI / 3 + Math.PI / 2//* pendulum2.l/2
          p5.circle(Math.cos(th) * pendulum1.l/2 + p5.width* 0.6, y, 3)
          p5.fill(BLUE)
          let th2 = Math.sin(t* Math.PI * pendulum2.freq + Math.PI) * Math.PI / 3 + Math.PI / 2//* pendulum2.l/2
          //let x2 = Math.sin(t* Math.PI * pendulum2.freq ) * pendulum2.l/2
          p5.circle(Math.cos(th2) * pendulum1.l/2 + p5.width* 0.6, y, 3)
          y += 1
        }
  ...
  SKETCHES = [
    s0_guitar(),
    pendulum_template(1, [2, 1], 's1_pendulum', 220),
    pendulum_template(1, [48, 27], 's2_pendulum', 220),
    pendulum_template(1, [3, 2], 's3_pendulum'),
    pendulum_template(1, [4,3], 's4_pendulum', 220),
  ]
